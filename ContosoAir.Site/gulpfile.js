...
gulp.task('serve', ['preprocessor-watch'], $.shell.task(['ng serve --target=development --port=4201']));

gulp.task('prebuild', ['preprocessor'], $.shell.task(['ng build --target=development']));

gulp.task('preprod', ['preprocessor'], $.shell.task(['ng build --target=production --aot=false']));
...
gulp.task('start2', ['prebuild'], copyWebConfig, function(){
...
gulp.task('documents', function () {
    return gulp.src('src/assets/documents/**/*')
        .pipe(gulp.dest('wwwroot/assets/documents'));
});
...
gulp.task('preprocessor', function(cb){
    runSequence('icons', 'styles','documents', cb);
});

gulp.task('preprocessor-watch', function(cb){
    runSequence('icons', 'styles','documents', 'watch', cb);
});